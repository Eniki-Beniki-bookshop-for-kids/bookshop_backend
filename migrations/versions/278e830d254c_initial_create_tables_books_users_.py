...
Revision ID: 278e830d254c
Revises: 
Create Date: 2025-03-17 21:02:07.889110
...
def upgrade() -> None:
    ...
    sa.Column('genre', sa.Enum('classics', 'fantasy', 'science_fiction', 'mystery', 'romance', 'non_fiction', 'coloring', 'fairy_tales', 'biography', 'history', 'poetry', 'self_help', 'business', 'travel', 'cooking', 'other_genre', name='genreenum'), server_default='other_genre', nullable=False),
    sa.Column('language', sa.Enum('ukrainian', 'english', 'russian', 'german', 'french', 'spanish', 'italian', 'polish', 'chinese', 'japanese', 'arabic', 'turkish', 'portuguese', 'dutch', 'swedish', 'latin', 'greek', 'hebrew', 'other_language', name='languageenum'), nullable=False),
    sa.Column('original_language', sa.Enum('ukrainian', 'english', 'russian', 'german', 'french', 'spanish', 'italian', 'polish', 'chinese', 'japanese', 'arabic', 'turkish', 'portuguese', 'dutch', 'swedish', 'latin', 'greek', 'hebrew', 'other_language', name='languageenum'), nullable=False),
    ...
    sa.Column('role', sa.Enum('admin', 'superadmin', 'user', name='userroleenum'), server_default='user', nullable=False),
    ...
    sa.Column('book_type', sa.Enum('paperback', 'Ebook', 'audiobook', name='booktypeenum'), nullable=False),
    sa.ForeignKeyConstraint(['book_id'], ['books.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('id')
    )
    op.create_index(op.f('ix_book_types_book_type'), 'book_types', ['book_type'], unique=False)
    ...
    sa.Column('paper_type', sa.Enum('offset', 'newsprint', 'writing', 'coated', 'vellum', 'cardboard', name='papertypeenum'), nullable=True),
    sa.Column('translator', sa.String(length=100), nullable=True),
    sa.Column('cover_type', sa.Enum('hard', 'soft', 'spiral', name='covertypeenum'), nullable=True),
    ...
    op.create_index(op.f('ix_books_info_cover_type'), 'books_info', ['cover_type'], unique=False)
    op.create_index(op.f('ix_books_info_original_title'), 'books_info', ['original_title'], unique=False)
    op.create_index(op.f('ix_books_info_paper_type'), 'books_info', ['paper_type'], unique=False)
    op.create_index(op.f('ix_books_info_publisher'), 'books_info', ['publisher'], unique=False)
    op.create_index(op.f('ix_books_info_translator'), 'books_info', ['translator'], unique=False)
    op.create_table('categories',
    sa.Column('id', sa.UUID(), nullable=False),
    sa.Column('book_id', sa.UUID(), nullable=False),
    sa.Column('category', sa.Enum('children_literature', 'young_adult', 'adult_literature', 'parents', 'other_category', name='categoriesenum'), nullable=False),
    sa.ForeignKeyConstraint(['book_id'], ['books.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('id')
    )
    ...
    op.create_table('target_ages',
    sa.Column('id', sa.UUID(), nullable=False),
    sa.Column('book_id', sa.UUID(), nullable=False),
    sa.Column('target_age', sa.Enum('age_1_3', 'age_3_5', 'age_5_8', 'age_8_12', 'teenager', 'adult', 'other_target', name='targetagesenum'), nullable=False),
    sa.ForeignKeyConstraint(['book_id'], ['books.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('id')
    )
    op.create_index(op.f('ix_target_ages_target_age'), 'target_ages', ['target_age'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_book_types_book_type'), table_name='book_types')
